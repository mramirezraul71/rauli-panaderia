# -*- coding: utf-8 -*-
"""
SETUP AUTOMÁTICO - CLOUDFLARE WORKER PARA CUBA
...
def create_worker_files():
    ...
    # package.json
    package_json = {
        "name": "rauli-cuba-proxy",
        "version": "1.0.0",
        "description": "Proxy Cloudflare Worker para RAULI ERP desde Cuba",
        "main": "src/worker.js",
        "scripts": {
            "dev": "wrangler dev",
        ...
        "keywords": ["cloudflare", "worker", "proxy", "cuba", "rauli-erp"],
...
def update_frontend_config(worker_url):
    ...
    proxy_config = f"""// Configuración Proxy Cuba - Cloudflare Worker
export const CUBA_PROXY_CONFIG = {{
  enabled: true,
...
"""
    ...
    config_path = BASE.parent / "frontend" / "src" / "config" / "cubaProxy.js"
...
def create_deployment_guide(worker_url):
    ...
    guide = f"""# 🚀 GUÍA DEPLOYMENT - PROXY CUBA
...
```javascript
import {{ CUBA_PROXY_CONFIG }} from '../config/cubaProxy';

// Ejemplo para Gemini
const response = await fetch(`${{CUBA_PROXY_CONFIG.workerUrl}}/api/gemini/models/gemini-1.5-flash:generateContent`, {{
  headers: {{
...
---
*Proxy configurado para operaciones desde Cuba*
"""
...
def main():
    ...
    print("🚀 SETUP AUTOMÁTICO - CLOUDFLARE WORKER CUBA")
    print("=" * 50)
    ...
    if worker_url:
        ...
        print("2. Prueba las APIs desde Cuba")
